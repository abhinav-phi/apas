...
export function FlowButton({ 
  ...
  const content = (
    <>
      ...
      {!hideArrow && (
        <ArrowRight 
          className={`absolute ${arrowSize} left-[-25%] ${strokeColor} fill-none z-[9] group-hover:left-4 ${groupHoverStroke} transition-all duration-800 ease-flow-2`} 
        />
      )}

      {/* Text */}
      <span className={`relative z-[1] ${hideArrow ? '' : 'group-hover:translate-x-3'} transition-all duration-800 ease-out font-mono tracking-wider uppercase`}>
        {text}
      </span>

      {/* Circle */}
      <span className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 ${circleColor} rounded-[50%] opacity-0 group-hover:w-[1500px] group-hover:h-[1500px] group-hover:opacity-100 transition-all duration-800 ease-flow-3`}></span>

      {/* Right arrow (arr-1) */}
      {!hideArrow && (
        <ArrowRight 
          className={`absolute ${arrowSize} ${rightArrowPos} ${strokeColor} fill-none z-[9] group-hover:right-[-25%] ${groupHoverStroke} transition-all duration-800 ease-flow-2`} 
        />
      )}
    </>
  );
